-- name = "aiofetch Version 2"
-- type = "widget"
-- description = "neofetch-like status information"
-- author = "ProfHasenbein"
-- version = "2.0"

local icon_android = [[
<?xml version="1.0" encoding="utf-8"?>
<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="220" height="220" viewBox="0 0 1024 1024">
    <path fill="#3ddc84" d="M782.259962 347.828081C690.71618 348.083344 599.172399 350.986964 507.628617 350.986964c-91.033255 0-182.034603 0-273.067858 0C223.935426 350.986964 224.828848 344.988278 224.828848 355.581702c0 131.396756 0 262.761604 0 394.15836 0 3.701317-0.350987 7.402634-0.095724 11.072043 2.456909 33.726656 31.652643 61.422719 65.475023 61.773706 13.177965 0.127632 26.292114 0.478619 39.438172-0.15954 7.402634-0.350987 9.061845 2.042106 8.998029 9.189477-0.350987 42.214159-0.255263 84.428319-0.15954 126.642478 0.031908 24.53718 9.827635 44.256265 31.110208 56.732257 42.724686 25.079614 95.149375-5.073357 95.436546-54.690151 0.287171-43.26712 0.15954-86.534241-0.063816-129.801361-0.031908-5.998686 1.244408-8.168424 7.753621-8.0727 22.941784 0.414803 45.915476 0.478619 68.857261-0.031908 7.338818-0.15954 8.647042 2.393093 8.615135 9.061845-0.287171 42.724686-0.127632 85.48128-0.127632 128.205966 0 5.264804 0.063816 10.561517 1.340132 15.730598 8.136516 32.354616 37.204618 51.850347 71.15463 47.702319 30.184879-3.701317 53.286203-30.823037 53.318111-62.858574 0.063816-40.108238 0-80.216475 0-120.292805 0-17.357901 0-17.166453 17.421717-17.421717 14.741452-0.223355 29.674352 1.946382 44.19245-1.340132 33.088498-7.498358 53.732913-35.258236 53.732913-71.537525 0.031908-131.907283-0.031908-262.314894 0.191447-394.222176C791.417531 348.115252 789.439241 347.828081 782.259962 347.828081zM939.789293 403.347837c0-5.520068-0.350987-11.072043-1.021053-16.528295-3.892765-32.00363-32.737511-55.392124-66.304628-53.924361-31.493103 1.37204-58.646731 28.525668-58.96581 60.17831-0.446711 44.830608-0.127632 89.661215-0.095724 134.491823 0 45.373042-0.350987 90.714176 0.127632 136.087218 0.287171 26.35593 20.197704 50.414491 45.660213 57.115151 41.831265 11.008228 80.535554-18.953296 80.59937-62.667127C939.885017 573.19362 939.821201 488.286683 939.789293 403.347837zM199.111076 378.204408c-9.029937-29.738168-43.841463-49.201991-73.802986-44.990147C93.049197 337.745183 65.289319 363.782034 65.289319 396.360006c0 44.03291 0 88.06582 0 132.09873 0 43.777647 5.041449 87.555294 5.105265 131.33294 0 4.211844 2.552632 8.519411 3.414146 12.571715 7.019739 33.311854 37.076987 53.605282 71.984236 50.063504 30.535866-3.095067 54.785874-30.280603 54.817782-62.826667 0.063816-87.810557 0.350987-175.621114 0.350987-263.43167C200.929826 390.106056 200.897918 384.075462 199.111076 378.204408zM777.601408 242.755347c-23.228955-63.783904-65.953641-110.688525-124.79182-143.426037-5.296712-2.967435-5.647699-4.88191-2.775988-9.955267 13.177965-23.388495 25.845404-47.096069 38.831921-70.580288 2.999343-5.392436 4.499015-10.433885-1.691119-14.390466-4.913817-3.158883-9.795727-0.893421-13.720399 6.190134-13.082241 23.77139-26.228299 47.510872-38.991461 71.441801-2.680264 5.041449-5.105265 5.807239-10.370069 3.573685-36.598368-15.507242-74.951671-22.877968-114.58129-23.133232-41.28883-0.287171-81.237528 7.051647-119.30366 23.292771-5.073357 2.169738-7.179279 1.116777-9.700003-3.573685-10.721056-20.229612-21.888823-40.235869-32.928959-60.33785-2.648356-4.850002-5.264804-9.731911-8.136516-14.454281-2.58454-4.275659-6.636844-5.169081-11.008228-3.446054-4.435199 1.723027-5.32862 5.647699-4.499015 9.827635 0.446711 2.265461 1.659211 4.403291 2.775988 6.477305 12.475991 22.909876 24.696719 45.915476 37.651329 68.538182 3.541778 6.190134 1.754935 8.008884-3.573685 11.072043C278.210774 147.574064 229.774573 217.356654 223.073913 314.005701c-0.478619 6.955923 1.180593 9.380924 8.806582 9.349016 91.543782-0.287171 183.055656-0.191447 274.599437-0.191447 91.799045 0 183.59809-0.095724 275.397135 0.15954 7.562174 0.031908 10.05099-1.659211 9.508556-9.476648C789.694504 289.532337 785.961279 265.72904 777.601408 242.755347zM377.540085 220.451721c-13.369413-0.319079-23.516127-11.199675-23.228955-24.888167 0.287171-13.177965 10.593425-22.973692 24.122377-22.846061 13.879939 0.127632 23.739482 10.21053 23.61185 24.154285C401.885817 210.336915 391.037129 220.802708 377.540085 220.451721zM636.153661 220.451721c-13.082241-0.15954-23.77139-11.295399-23.61185-24.632903 0.15954-13.018426 10.625333-23.101324 23.962837-23.101324 13.656584 0 24.026653 10.338161 23.994745 23.930929C660.531302 209.922112 649.554982 220.611261 636.153661 220.451721z" />
</svg>
]]


local icon_arch = [[
<?xml version="1.0" encoding="utf-8"?>
<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="220" height="220" viewBox="0 0 48 48">
    <path fill="#1e88e5" d="M28.465,38.611c0.419-1.105,0.664-2.365,0.664-3.714c0-4.133-2.211-7.494-4.929-7.494
        c-2.741,0-4.951,3.361-4.951,7.494c0,1.326,0.221,2.586,0.641,3.669c-9.041,0.951-15.407,4.731-17.993,6.432
        c4.355-6.278,8.909-13.638,13.262-22.105c1.083-2.101,2.101-4.178,3.05-6.211c0.375,0.243,0.751,0.509,1.171,0.775
        c1.945,1.215,3.759,1.879,5.084,2.233c-0.973-0.73-2.033-1.613-3.116-2.697c-0.817-0.817-1.547-1.637-2.167-2.433
        C21.016,10.538,22.608,6.669,24,3c2.32,6.144,5.217,12.842,8.841,19.893c2.343,4.531,4.731,8.754,7.117,12.644
        c-0.685-0.375-1.437-0.73-2.233-1.039c-1.371-0.53-2.652-0.862-3.759-1.06c1.503,0.751,3.25,1.747,5.084,3.073
        c1.194,0.885,2.254,1.769,3.161,2.631c0.021,0.021,0.021,0.021,0.045,0.045c1.26,2.056,2.565,3.957,3.846,5.813
        C43.561,43.319,37.306,39.605,28.465,38.611z"/>
</svg>
]]

network_result_received = false
start_time = nil
network_result = "Loading..."


local battery_percent = ""
local battery_temp = ""
local battery_voltage = ""
local battery_status = ""

local sys_storage_total = ""
local sys_storage_free = ""
local sys_connection = ""
local sys_launcher = ""
local sys_device = ""
local sys_android = ""
local sys_mem_total = ""
local sys_mem_free = ""
local sys_ssd_total = ""
local sys_ssd_free = ""

function on_resume()
    network_result_received = false
    start_time = os.time()
    http:get("https://api.ipify.org?format=text")
end

function on_tick(ticks)
    if ticks % 2 == 0 then
        network_result_received = false
        start_time = os.time()
        http:get("https://api.ipify.org?format=text")
    end

    if not network_result_received and start_time and (os.time() - start_time) >= 1 then
        on_network_result("not connected", 0)
    end
end

function on_network_result(result, code)
    if network_result_received then return end
    network_result_received = true

    if not code or code < 200 or code >= 300 then
        result = "not connected"
    end
    network_result = result
    update_gui()
end

function update_gui()
    local lang_text = "Language: " .. system:lang()

    local batt_info = system:battery_info()
    local sys_info = system:system_info()

    local battery_percent = "Battery: " .. batt_info.percent .. "%"
    local battery_temp = "Temp: " .. batt_info.temp .. "°C"
    local battery_voltage = batt_info.voltage .. " mV"
    local battery_status = batt_info.status

    local sys_storage_total = sys_info.storage_total
    local sys_storage_free = sys_info.storage_available
    local sys_connection = "Connection: " .. sys_info.connection
    local sys_launcher = "Launcher: " .. sys_info.aio_version
    local sys_device = "Host: " .. sys_info.device
    local sys_android = "OS: Android " .. sys_info.android_version
    local sys_mem_total = sys_info.mem_total
    local sys_mem_free = sys_info.mem_available
    local sys_ssd_total = sys_info.sdcard_total
    local sys_ssd_free = sys_info.sdcard_available

    local sys_storage_unit = sys_storage_total:gsub("[^%a]", "")
    local sys_storage_max = sys_storage_total:gsub("[^0-9%.]", "")
    local sys_storage_free_val = sys_storage_free:gsub("[^0-9%.]","")
    local storage_used = sys_storage_max - sys_storage_free_val
    local sys_storage_text = "Storage: " .. storage_used .. " " .. sys_storage_unit .. " / " .. sys_storage_total

    local sys_mem_unit = sys_mem_total:gsub("[^%a]", "")
    local sys_mem_max = sys_mem_total:gsub("[^0-9%.]", "")
    local sys_mem_free_val = sys_mem_free:gsub("[^0-9%.]","")
    local mem_used = sys_mem_max - sys_mem_free_val
    local sys_mem_text = "Memory: " .. mem_used .. " " .. sys_mem_unit  .. " / " .. sys_mem_total

    local divider = "---------------------------------------------<br>"

    my_gui = gui{
        {"spacer", 2},
        {"icon", "svg:"..icon_arch, {size = 170}},
        {"spacer", 4},
        {"text", " ‍  ‍  ‍ " .. "%%fa:user%% nibblus@android<br>"
          .. "╭─" .. sys_android .. "<br>"
          .. "├─" .. lang_text .. "<br>"
          .. "├─" .. sys_device .. "<br>" 
          .. "╰─" .. "IP: " .. network_result .. "<br>"
	      .. "╭─" .. sys_mem_text  .. "<br>"
          .. "├─" .. sys_storage_text .. "<br>"
          .. "├─" .. battery_percent .. "<br>"
          .. "╰─" .. battery_temp,
        {color = "#ffffff"}
        }
    }
    my_gui.render()
end